  ...
  return (
    <form className='places__sorting' action='#' method='get'>
      ...
      <ul ref={dropDownRef} className='places__options places__options--custom'>
        {SortTypes.map((sortType) => <li className={`places__option ${currentSort===sortType ? 'places__option--active' : ''}`} tabIndex='0' key={sortType} onMouseEnter={(event) => {event.target.classList.add('places__option--active');}} onMouseLeave={(event) => { if (currentSort !== sortType) {event.target.classList.remove('places__option--active');}}} onClick={() => { onSortClick(sortType); handleDropDownClose(dropDownRef);}}>{sortType}</li>,
        )}
      </ul>
    </form>
  );
